...
/**
 * GET /api/vouchers
 * Get ALL vouchers (Sales, Receipt, Payment, Journal, Purchase, etc.)
 */
 ...
 */
router.get('/vouchers/types', (req, res) => {
  try {
 ...
 */
router.get('/fonepay/unlinked', (req, res) => {
  try {
 ...
 */
router.get('/fonepay/for-bill/:voucherNumber', (req, res) => {
  try {
 ...
 */
router.post('/rbb/sync', async (req, res) => {
  try {
    // Run sync in background
 ...
 */
router.get('/rbb/transactions', (req, res) => {
  try {
 ...
 */
router.get('/rbb/summary', (req, res) => {
  try {
 ...
 */
router.post('/rbb/start', async (req, res) => {
  try {
    rbbService.start();
 ...
 */
router.post('/rbb/stop', async (req, res) => {
  try {
 ...
 *       chequeNumber: string (optional - can add later),
 *       chequeDate: string YYYYMMDD (optional - can confirm later),
 *       branch: string (optional)
 *     }
 *   ]
 * }
 */
router.post('/cheque-receipt-activity', async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    if (io) {
      io.emit('chequeReceipt:created', { voucherNumber, partyName, totalChequeAmount, chequeCount: cheques.length });
    }
...
/**
 * PUT /api/cheque-receipt-activity/:chequeId/update-date
 * Update cheque date and sync to Tally
 */
router.put('/cheque-receipt-activity/:chequeId/update-date', async (req, res) => {
  try {
...
/**
 * POST /api/cheque-receipt-activity/:chequeId/add-breakdown
 * Add another cheque to same bill (breakdown)
 */
router.post('/cheque-receipt-activity/:chequeId/add-breakdown', async (req, res) => {
  try {
...
/**
 * GET /api/cheque-receipt-activity/pending-dates
 * Get all cheques needing date confirmation, grouped by bill
 */
router.get('/cheque-receipt-activity/pending-dates', (req, res) => {
  try {
 ...
 *   bankName: string (required),
 *   amount: number (required),
 *   chequeNumber: string (optional - can be added later),
 *   chequeDate: string YYYYMMDD (optional - can be confirmed later),
 *   branch: string (optional),
 *   narration: string (optional),
 *   voucherNumber: string (optional - link to bill),
 *   billId: number (optional - link to bill),
 *   pushToTally: boolean (default: true)
 * }
 */
 ...
 */
router.get('/cheques/pending', (req, res) => {
  try {
 ...
 */
router.get('/cheques/due-today', (req, res) => {
  try {
 ...
 */
router.get('/cheques/needs-date', (req, res) => {
  try {
    const cheques = db.getPendingDateConfirmations();
    const total = cheques.reduce((sum, c) => sum + c.amount, 0);
 ...
 */
router.get('/cheques/unsynced', (req, res) => {
  try {
 ...
 */
router.get('/cheques/summary', (req, res) => {
  try {
 ...
 *
 * Body: {
 *   chequeDate: string YYYYMMDD (required),
 *   chequeNumber: string (optional),
 *   userId: number (optional)
 * }
 */
router.put('/cheques/:id/confirm-date', async (req, res) => {
  try {
 ...
 *
 * Body: {
 *   status: 'pending' | 'deposited' | 'cleared' | 'bounced' (required),
 *   depositDate: string (for deposited status),
 *   clearDate: string (for cleared status),
 *   bounceDate: string (for bounced status),
 *   bounceReason: string (for bounced status)
 * }
 */
 ...
 */
router.post('/cheques/sync-pending', async (req, res) => {
  try {
    // Check Tally connection
    const connectionStatus = await tallyConnector.checkConnection();
    if (!connectionStatus.connected) {
 ...
 * Shows all cheques + summary (pending, deposited, cleared, bounced)
 */
router.get('/cheques/party/:partyName', (req, res) => {
  try {
 ...
 */
router.get('/cheques/bill/:voucherNumber', (req, res) => {
  try {
 ...
 *   bankDeposit: number,
 *   notes: string,
 *   cheques: [{ bankName, amount, chequeNumber?, chequeDate? }] (optional)
 * }
 */
 ...
 */
router.get('/bill-payments/:voucherNumber', (req, res) => {
  try {
    const voucherNumber = decodeURIComponent(req.params.voucherNumber);
    const payment = db.getBillPayment(voucherNumber);
 ...
 */
router.get('/bill-payments/partial', (req, res) => {
  try {
